...
function mapRequestToUi(request) {
    return {
        id: request.id,
        doctor: `Doctor #${request.doctor_user_id}`,
        specialty: 'HealthConnect',
        hospital: 'Consent Layer',
        date: new Date(request.requested_at).toLocaleDateString('en-CA'),
        reason: request.reason,
        requestedCategories: request.requested_categories,
    ...
    useEffect(() => {
        ...
        async function loadData() {
            try {
                ...
                if (Array.isArray(recordsResponse) && recordsResponse.length > 0) {
                    setRecords(recordsResponse.map(mapRecordToUi))
                }

                if (Array.isArray(requestsResponse)) {
                    const pending = requestsResponse.filter((item) => item.status === 'pending')
                    if (pending.length > 0) {
                        setRequests(pending.map(mapRequestToUi))
                    }
                }
    ...
    async function handleRequest(id, action) {
        ...
        try {
            ...
            if (target) {
                if (action === 'approved') {
                    await approveAccessRequest({
                        requestId: id,
                        approvedCategories: target.requestedCategories ?? null,
                        durationHours: target.requestedDurationHours ?? null,
            ...
            setToast({
                id,
                action: 'denied',
                message: error?.message || 'Could not update consent request',
            })
        } finally {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-gray-700 text-xs">{isGuardian ? 'Guardian Portal' : 'Patient Portal'}</p>
                        </div>
                    </div>
